...
class CredentialResolver
{
    ...
    /**
     * WhatsApp Web (QR / unofficial) engine — WAHA base URL + API key, plus the
     * webhook HMAC secret from the config's separate `webhook_secret` column.
     */
    public function whatsappWeb(): ?WhatsappWebCredentials
    {
        $config = IntegrationConfig::forProvider('whatsapp_web');
        if (! $config || ! $config->enabled) {
            return null;
        }
        ...
        return new WhatsappWebCredentials(array_merge($creds, [
